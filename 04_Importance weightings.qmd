...
This report should be shared with all respondents. Our approach should be transparent and accessible to the respondents, in particular to give a working understanding of [how condition scores will be estimated from indicator measurements](01_general%20approach.qmd) and thus promote effective discussion and deliberation in forthcoming workshops.
...
-   Relying more on order that respondents ranked indicators, rather than the raw scores themselves, and building consensus based on that (initial exploration below: @sec-introduction ).
...
#add a col to just.one.df that is the proportion of all of each respondents weight given to each indicator
just.one.df <- just.one.df %>%
  group_by(respondant_name) %>%
...
# just some background lines to help read the graph
  ...
  indiv.weights <- ggplot() +
    ...
    guides(color = guide_legend(ncol =1), alpha = F) +
...
```{r shares of total weights ggiraph, caption = "Weightings shares: percentage of total weights given to each indicator, boxplots show the median and inter-quartile range for each indicator's weighting share"}

  # Order the levels of sheet_name by the median of rank_weight
ordered_levels <- just.one.df %>%
  group_by(sheet_name) %>%
  summarise(median_rank = median(weight_share, na.rm = TRUE)) %>%
  arrange(-median_rank) %>%
  pull(sheet_name)
...
# just.one.df$sheet_name <- factor(just.one.df$sheet_name, levels = ordered_levels)
  ...
  indiv.weights <- ggplot(data = just.one.df %>% 
    ...
    geom_point_interactive(position = position_jitter(width = 0.2, height = 0),
                           size = 2, shape = 16,
                           aes(x = sheet_name, y = weight_share,
                               col = respondant_name,
                               alpha = cert_weight,
                               tooltip = paste0(respondant_name, "<br>",sheet_name, "<br>Weight = ", weight, "<br>Weight rank = ", rank_weight, "<br> Weight share = ", weight_share, "<br>Certainty = ", cert_weight),
                               data_id = respondant_name #,
                               # text = map(
                               #   paste0("<b>", respondant_name, "</b><br>",
                               #          "<b>Weight: </b>", weight, " (certainty ", cert_weight,")<br>"),
    ...
    geom_boxplot(aes(x = sheet_name, y = weight_share), 
             width=0.5, color="grey30", fill = NA,
               outlier.shape = NA) +
    scale_colour_manual(values = respondant_colours, name = "Respondant") +
    # scale_y_continuous(limits = c(1,14)) + 
    guides(color = guide_legend(ncol =1)) +
    labs(y = NULL, x = NULL, title = "Indicators weighting shares") +
    theme_pubr()+
    theme(plot.title = element_text(size = 12),
          plot.subtitle = element_text(size = 12),
          legend.title = element_text(size = 12 , face = "bold"),
...
## Ranked weightings {#sec-introduction}


## Share of total weightings given to each indicator

The ranks given to indicator importance also exhibited a lot of variation between respondents, although there are a few general trends worth noting (to which there are many exceptions):

-   Deadwood and horizontal complexity were generally ranked very highly
-   Regen, native canopy percentage and
-   Tree health and woodland extent generally ranked low
-   
...
column_names <- c("Indicator", "Average weight", "Average weight rank (lower better)", "Average weight share (higher better)*","Ranked most important \n(% responants)", "Ranked least important (% responants)"
...
DT::datatable(just.one.df %>%
                ...
                summarise(
                  ...
                  av_weight_share = round(mean(weight_share, na.rm = T), 2),
...
```{r heatmap share weights, echo = F, results="asis"}
generate_interactive_heatmap(just.one.df, "weight_share")
```
:::
