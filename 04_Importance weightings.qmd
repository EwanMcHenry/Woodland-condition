...
Weightings define the importance of different indicators, or the amount each contributes towards a condition score, relative to other indicators (more information and a toy example in [General approach](01_general%20approach.qmd)).
...
There a lot of variation between respondents for [raw indicator weightings](#sec-raw_weights), much more than there was between actual indicators. This may have be partly because respondents varied a lot in terms of the [amount of difference they gave between indicators](#variation%20in%20raw%20indicators). This limited what useful information could be drawn from raw weightings, and prompted an exploration of the [ranking of indicator weightings](#sec-rank_weights) by respondents and [Standardised weights](#sec-standardised_weights) (transformed so that all respondents varied the same amount between indicators).

Many respondents provided weighitng estimates that were very similar between indicators. The implications of this are discussed [below](#variation%20in%20raw%20indicators). It may be desirable to encourage larger differences between indicators of greater and lesser importance.
...
## Respondents varied in their difference between indicators {#sec-variation-in-raw-indicators}

Respondents varied in terms of [how much difference]{.underline} they gave between raw indicator weights. The figure below shows how much variation there typically was between weightings for each respondent. Respondents with lower standard deviations weighted indicators more similarly, while those with higher tended to weight indicators differently to each other.
...
p_respondant_sd <- ggplot(respondant.sd, aes(y = sd_weight_respondant, x = 0)) +
...
The figures below show the weightiness for each indicator given by respondents:

-   Raw weightings

> Note: these varied a lot between respondents for each indicator, much more than between the indicators themselves. This limits how useful the first round of estimated raw weightings will be to differentiate between different indicators importance.

-   Standardised estimates (transformed so that all respondent's answers varied the same amount[^04_importance-weightings-1])

-   The rank of those estimates by respondent
