...
table_data <- approp_ground_flora_spp %>%
  
  # convert habitat columns into:
  #   habitat_type
  ...
  pivot_longer(
    ...
    values_to = "appropriate"
  ) %>%
  
  # keep only species marked as appropriate
  filter(appropriate) %>%
  
  # split habitat_type into:
  #   soil_hydrology
  #   habitat
  #
  # e.g.
  # acidic_upland ->
  #   soil_hydrology = acidic
  #   habitat = upland
  separate(
    habitat_type,
    into = c("soil_hydrology", "habitat"),
    sep = "_(?=[^_]+$)"
  ) %>%
  ...
  # group by habitat combination
  group_by(habitat, soil_hydrology) %>%
  
  # calculate:
  #   n_species = number of appropriate species
  #   species_list = species names for tooltip
  summarise(
    n_species = n(),
...
table_data %>%
  ...
  # make species count show tooltip on hover
  mutate(
    n_species = text_spec(
      n_species,
      "html",
      tooltip = species_list
    )
  ) %>%
  ...
  rename(
    ...
    "Soil/hydrology" = soil_hydrology,
    "N appropriate species" = n_species
  ) %>%
