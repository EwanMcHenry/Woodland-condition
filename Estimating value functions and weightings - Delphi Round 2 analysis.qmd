...
source("Scripts\\functions_estiamting_value_functions_weights.R") # contains functions for round 2 analysis
...
:::: toggle-section
<button onclick="toggleVisibility(&#39;my-section&#39;)">

Show/hide curation info

</button>

::: {#my-section style="display: none; margin-top: 1em;"}
...
The original expert opinion elicitation was done assuming shorter lists of appropriate species. The list has since been expanded, to the point where all individual tree/shrub species could not fit in the plot.

To allow for this expanded lists, while maintaining pragmatic ecological realism, I have transformed the value function, reducing its range along the measurements and assuming max score beyond that. I multiplied the original measures by 0.6 (the approximate mean of the relative change in list sizes for each habitat type) and manually set the value for all measures above 60% to 100.

```{r n_tree_shrub_spp_transform, warning=FALSE, message=FALSE}
# Transform the measure to fit the new list size
to_addback <- data.frame(measure = newdata$measure[newdata$measure > 60],
                                  value_plot = 100)

newdata <- newdata %>%
  mutate(measure = measure * 0.6) %>% 
  select(measure, value_plot) %>%
  rbind(to_addback)
