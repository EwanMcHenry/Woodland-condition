...
# FUNCTION - PLOT RESPONDANTS' WEIGHTS - FOR INDICATOR  ----
  ggplot_resp_weight_ind <- function(indicator_name, this.ind.num = ind.num){
...
## Color palette
  respondant_colours <- glasbey.colors(nlevels(as.factor(weights_filtered_data$respondant_name))+1)[-1] %>% as.character()
  
weights_filtered_data$respondant_colour.indf = respondant_colours[as.numeric(weights_filtered_data$respondant_name)]
...
# Filter the data for the selected indicator
  ...
  # add interactivity for hover to show that respondants weights, with this one highlighted
  

# plot of all indicator weights for all respondants, by indicator, interactive  
...
girafe(
  ggobj = plot_grid(plot, indiv.weights),
  width_svg = 15,
  height_svg = 6, 
  options = list(
...
plotly_p <- ggplotly(wt.plot, tooltip = "text", dynamicTicks = F) %>% 
    config(displayModeBar = F) %>% 
    layout(yaxis = list(range = c(-5, 105)))
