...
# FUNCTION - PLOT RESPONDANTS' INDICATOR VFs ----
  plot_gg_gam <- function(indicator_name, gam.col = "black", x.lab = ind.matcher.df$ind.axis.title[i]){
...
```{r vf Tree age size distribution, echo = F , results="asis"}
...
# filtered_data %>% 
#   group_by(as.factor(measure)) %>% 
#   length()

plot <- plot_gg_gam(indicator_name = indicator_name,
...
for(i in 1: length(ind.matcher.df$indicator_name)){
  ...
  plot <- plot_gg_gam(indicator_name = ind.matcher.df$indicator_name[i])
