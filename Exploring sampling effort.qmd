...
max_plots <- 12
min_plots <- 3
iterations <- 500  # change as needed
run.sim = F
...
# remove "outputs/survey data summaries/all_trial_zone_survey_data.RData"
  .[!grepl("all_trial_zone_survey_data.RData", .)]
...
for (i in 1:length(summary_files)) {
  ...
  treespp <- data.frame(
    ...
    plot_number = rep(all_summaries$tree_spp_summary$combined_table$plot, 4),
    indicator_name = "N tree & shrub spp.",
    var_type = c(rep("N", nrow(all_summaries$tree_spp_summary$combined_table)),
                 rep("N appropriate species", nrow(all_summaries$tree_spp_summary$combined_table)),
                 rep("Per of appropriate species", nrow(all_summaries$tree_spp_summary$combined_table)),
                 rep("Value", nrow(all_summaries$tree_spp_summary$combined_table))),
    var = c(all_summaries$tree_spp_summary$combined_table$spp_n,
            all_summaries$tree_spp_summary$combined_table$N_appropriate_species,
            all_summaries$tree_spp_summary$combined_table$per_of_appropriate_species,
            all_summaries$tree_spp_summary$combined_table$value)
  )
  ### regeneration classes ----
  regen <- data.frame(
    # plot_type = rep(all_summaries$regen_summary$plot_type[all_summaries$regen_summary$plot_type== "main"], 2),
    plot_number = rep(all_summaries$regen_summary$plot[all_summaries$regen_summary$plot_type== "main"], 2),
    indicator_name = "Regen",
    var_type = c(rep("N", sum(all_summaries$regen_summary$plot_type== "main")),
                 rep("Value", sum(all_summaries$regen_summary$plot_type== "main"))),
    var = c(all_summaries$regen_summary$regen_classes_n[all_summaries$regen_summary$plot_type== "main"],
            all_summaries$regen_summary$value[all_summaries$regen_summary$plot_type== "main"])
  )
  ...
  summary_dataframes_site <- rbind(
    ...
    select(site_name, zone, habitat_type, plot_number,
...
save(summary_df, file = "outputs/survey data summaries/all_trial_zone_survey_data.RData")
...
for (this_site_zone in site_zones) {
  ...
  zone_results <- map((min_plots-1):max_plots, function(n_plots) {
...
save(overall_scores_df, indicator_mean_values_df, file = "outputs/sampling effort simulation objects.RData")

}

```

```{r summarise sim results, warning=F, message=F}

load("outputs/sampling effort simulation objects.RData")
...
all_overall_scores_summary <- overall_scores_df %>%
  ...
  mutate(ci_range_95 = upper_95 - lower_95,
         ...
         lower_95_offset = mean_overall_score - lower_95,
         upper_95_offset = upper_95 - mean_overall_score,
         lower_80_offset = mean_overall_score - lower_80,
         upper_80_offset = upper_80 - mean_overall_score,
         lower_50_offset = mean_overall_score - lower_50,
         upper_50_offset = upper_50 - mean_overall_score)

# offsets across sites for sample size plots ----
offest_df <- all_overall_scores_summary %>%
  group_by(n_plots) %>%
  summarise(
    lower_50_offset_mean = mean(lower_50_offset, na.rm = TRUE),
    upper_50_offset_mean = mean(upper_50_offset, na.rm = TRUE),
    lower_80_offset_mean = mean(lower_80_offset, na.rm = TRUE),
    upper_80_offset_mean = mean(upper_80_offset, na.rm = TRUE),
    lower_95_offset_mean = mean(lower_95_offset, na.rm = TRUE),
    upper_95_offset_mean = mean(upper_95_offset, na.rm = TRUE),
    lower_50_offset_max = max(lower_50_offset, na.rm = TRUE),
    upper_50_offset_max = max(upper_50_offset, na.rm = TRUE),
    lower_80_offset_max = max(lower_80_offset, na.rm = TRUE),
    upper_80_offset_max = max(upper_80_offset, na.rm = TRUE),
    lower_95_offset_max = max(lower_95_offset, na.rm = TRUE),
    upper_95_offset_max = max(upper_95_offset, na.rm = TRUE),
    lower_50_offset_min = min(lower_50_offset, na.rm = TRUE),
    upper_50_offset_min = min(upper_50_offset, na.rm = TRUE),
    lower_80_offset_min = min(lower_80_offset, na.rm = TRUE),
    upper_80_offset_min = min(upper_80_offset, na.rm = TRUE),
    lower_95_offset_min = min(lower_95_offset, na.rm = TRUE),
    upper_95_offset_min = min(upper_95_offset, na.rm = TRUE),
    .groups = 'drop') 
...
-   A provisional method for the Woodland Trust's Woodland Ecological Condition Assessment (WECA) has been developed, with significant input from dozens of practitioners and experts across the Trust.

-   Field trials have been completed at multiple sites.

-   An analytical pipeline has been developed to take preliminary excel field forms and produce WECA reports for surveyed zones, including info on:

    -   The overall WECA summary score for the zone.

    -   Detail on the contribution of different indicators, and their variation between plots etc.

This report presents evidence to a answer **key questions that remain:**

-   What allocation of sampling effort is appropriate for the method?

-   How many plots should be surveyed within zones to best balance the competing demands of practical survey constraints and analytical rigor?

-   Does this requirement vary between different zones?

# Method

Where possible, field trial sites were intentionally "over sampled", surveying more plots that was thought would typically be practically possible.
...
Here, I explored the impact of plot number on survey result uncertainty by randomly re-sampling the surveyed plots (with replacement), differing the numbers of plots to simulate different sampling intensities (from 1 to `r max_plots` plots). I did this `r iterations` times for each zone and number of plots, and and calculated a WECA summary score and individual indicator scores each time.

Uncertainty in WECA scores under different sampling efforts is represented by the variation in these re-sampled scores.

I present:

-   A plot of the **mean scores** for each zone-sampling effort combination across all simulated iterations, with quantile ranges representing uncertainty, to demonstrate the estimation of the WECA summary scores in different zones and how the uncertainty generally decreases with sampling effort.

-   A plot of the **sizes of the quantile ranges**[^1], focusing more explicitly on how the amount of uncertainty varies with different sample sizes at different zones and on average (predicted using a GAM).

-   A plot of the marginal reduction in the quantile ranges, representing **the value added by each plot**: the amount that quantile ranges are reduced by adding each plot.

[^1]: **e.g. a 50% quantile range is the range of the most-central 50% of simulated results.**
...
## Variation in summary score estimates

### Intervals around mean score

```{r intervals around mean score, warning=F, message=F, caption = "Variation in WECA summary score estimates (quantile ranges) against number of plots. a) 50% quantile offsets. b) 80% quantile offsets."}
# plot of offsets around mean score ----
## ploting 50 and 80% quantile offsets, with sites coloured
## overlay with mean offsets in black


ci_50_p <- ggplot(all_overall_scores_summary) +
  geom_line(aes(x = n_plots, y = -lower_50_offset, color = site_zone), alpha = 0.6) +
  geom_line(aes(x = n_plots, y = upper_50_offset, color = site_zone), alpha = 0.6) +
  labs(title = "Confidence intervales: 50% quantile", y = NULL, x = NULL, color = "Site-Zone") +
  scale_color_brewer(palette = "Set1") +
  scale_x_continuous(breaks = pretty_breaks()) +
  theme_minimal() +
...
ci_80_p <- ggplot(all_overall_scores_summary) +
  geom_line(aes(x = n_plots, y = -lower_80_offset,
                color = site_zone), alpha = 0.6) +
  geom_line(aes(x = n_plots, y = upper_80_offset,
                color = site_zone), alpha = 0.6) +
  labs(title = "Confidence intervales: 80% quantile", y = NULL,
       x = NULL, color = "Site-Zone") +
  scale_color_brewer(palette = "Set1") +
  scale_x_continuous(breaks = pretty_breaks()) +
  theme_minimal() +
  theme(legend.position = "none",
...
legend <- get_legend(ggplot(all_overall_scores_summary, 
                            aes(x = n_plots, y = upper_50_offset, color = site_zone)) +
    geom_line() +
    theme_minimal() +
    theme(legend.position = "bottom")
)
# Combine plots side by side
plots <- plot_grid(ci_50_p, ci_80_p, nrow = 1
                  , align = "v", axis = "lr")
# Add shared axis labels
final_plot <- ggdraw(plots) +
  draw_label("Confidence around mean score", x = 0.015, y =
             0.5, angle = 90, size = 11, hjust = 0.5, vjust = 0.5) +
  draw_label("Number of plots", x = 0.5, y = 0
             , size = 11, hjust = 0.5, vjust = 0.5)
# Add legend below
final_plot_with_legend <- plot_grid(final_plot, legend, ncol = 1,
                                    rel_heights = c(1, 0.1))
...
p50 <- ggplot(all_overall_scores_summary, aes(x = n_plots, y = ci_range_50, color = site_zone)) +
  ...
  labs(title = "a) 50% quantile size", y = NULL, x = NULL, color = "Site-Zone") +
  scale_color_brewer(palette = "Set1") +
  scale_x_continuous(breaks = pretty_breaks()) +
  theme_minimal() +
  theme(legend.position = "none",
...
p80 <- ggplot(all_overall_scores_summary, aes(x = n_plots, y = ci_range_80, color = site_zone)) +
  ...
  labs(title = "b) 80% quantile size", y = NULL, x = NULL, color = "Site-Zone") +
  scale_color_brewer(palette = "Set1") +
  scale_x_continuous(breaks = pretty_breaks()) +
  theme_minimal() +
  theme(legend.position = "none",
...
marginal_reduction <- all_overall_scores_summary %>%
  ...
  group_by(site_zone) %>%
  mutate(marginal_reduction_50 = abs(lag(ci_range_50) - ci_range_50),
         marginal_reduction_80 = abs(lag(ci_range_80) - ci_range_80),
         marginal_reduction_95 = abs(lag(ci_range_95) - ci_range_95)) %>%
  ungroup()
