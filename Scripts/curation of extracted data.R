...
) %>% merge(., ind.matcher.df, by.x = "indicator_name", by.y = "indicator_name_topof_sheet", all.x = T) %>%   # this is wrong -- its sheet name.. but its fine
...
has_completed <- data.frame(respondent_names = respondent_names,
  ...
  left_join(., ind.matcher.df[, c("sheet_name", "indicator_name_topof_sheet")], by = c("indicator_name" =  "indicator_name_topof_sheet"))
