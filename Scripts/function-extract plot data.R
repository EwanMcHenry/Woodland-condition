...
extract_plot_survey_data <- function(
  ...
  for(this.plot in these.plots){ # test: this.plot <- 1
    ...
    main_willowed_list <- ifelse(
      str_detect(trees_main_df$tree_species[trees_main_df$spp_total_trees>0], regex("willow", ignore_case = TRUE)),
      "willow sp.",
      trees_main_df$tree_species[trees_main_df$spp_total_trees>0]
    ) %>% unique()
    supp_willowed_list <- ifelse(
      str_detect(trees_supp_df$tree_species[trees_supp_df$spp_total_trees>0], regex("willow", ignore_case = TRUE)),
      "willow sp.",
    ...
    ind.tree_health.max.dieback_mortality <- if (all(is.na(c(ind.tree_health.dieback.measure, 
                                                             ind.tree_health.mortality.measure)))) {
      0
    } else {
      max(ind.tree_health.dieback.measure, ind.tree_health.mortality.measure, na.rm = TRUE)
    }    
    ## affected species
    # TODO
