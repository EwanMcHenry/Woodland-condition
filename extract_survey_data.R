...
# Config
excel_sheet_path = "Data\\Field Test\\Ladypark Wood, Wye valley - Non-intervention 70yrs broadleaf SSSI ASNW\\WCA - Lady park wood - Field test - 12.06.2025.xlsx"
these.plots = 1
write_plot.csv = F
...
for(this.plot in 1:length(plot_sheets)){
  # write plot data if wanted ---- 
  if(write_plot.csv == T){
    write.csv(plot_data[[this.plot]], paste0("Data\\Field Test\\Ladypark Wood, Wye valley - Non-intervention 70yrs broadleaf SSSI ASNW\\plot", this.plot, ".csv"), row.names = FALSE)
  }
  ...
  # nativeness ----
  ind.nativeness.measure <- plot_data[[this.plot]][position.nativeness.measure[1], position.nativeness.measure[2]]
