...
point_size = 2
```
```{r f_indicator_summarising_functions, echo=FALSE, message=FALSE, warning=FALSE}
...
create_ground_flora_summary <- function(){
  ...
  wide_cover <- data.frame(
    species = ind$ground_flora_df$groundflora_species,
    cover = ind$ground_flora_df$cover,
    plot = i
  ) %>%
    tidyr::pivot_wider(
      names_from = species,
...
Detailed tree and shrub species, age and regeneration data was collected at each plot and supplementary plot. This info is used to calculate multiple indicators.

> The tree survey data can be explored by expanding the plot info below.
