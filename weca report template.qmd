...
zone_summary <- data.frame(
  ...
  value = c(
    ...
    mean(all_summaries$tree_spp_summary$combined_table$value, na.rm = TRUE),
...
`r circle_badge(round(mean(tree_species_summary$combined_table$value, na.rm = TRUE), 2))`

The mean tree species richness value across the zone is `r round(mean(tree_species_summary$combined_table$value, na.rm = TRUE), 2)`.
...
The appropriate tree and shrub species list for this habitat type (`r survey_data[[1]]$survey_info$habitat_type`) has `r length(survey_data[[1]]$indicators$tree_spp$list_appropriate_tree_spp)` species in it:

::: {style="columns: 3; -webkit-columns: 3; -moz-columns: 3;"}
<ul>`r paste0("<li>", sort(survey_data[[1]]$indicators$tree_spp$list_appropriate_tree_spp), "</li>", collapse = "\n")`</ul>
:::

```{r tree_species_richness_plot, warning=FALSE, message=FALSE}
ggplot() +
  geom_line(
    data = tree_spp_lookup,
    aes(x = per_spp, y = value),
    color = "black", size = vf_size, alpha = vf_alpha
  ) +
  geom_point(
    data = tree_species_summary$combined_table,
    aes(
...
```{r both_tree_species_richness_table, results='asis'}
appropriate_species <- survey_data[[1]]$indicators$tree_spp$list_appropriate_tree_spp

species_cols <- setdiff(names(tree_species_summary$combined_table),
                        c("plot", #"plot_type",
                          "value", 
...
tree_species_summary_reordered <- tree_species_summary$combined_table %>%
  select(plot, value, per_of_appropriate_species,
...
datatable(
  ...
  rownames = FALSE,
  caption = "Tree Species Richness Summary by Plot",
  options = list(
    pageLength = 20,
    dom = 'ftip',
    ordering = FALSE,
  ...
  formatStyle(
    ...
    backgroundColor = styleInterval(0, c(NA, "gold"))
  )
```

```{r seperate_tree_species_richness_table, results='asis'}

species_cols <- setdiff(names(tree_species_summary$separate_table),
...
# Rebuild data with species columns in new order
tree_species_summary_reordered <- tree_species_summary$separate_table %>%
  select(plot, plot_type, N_appropriate_species, spp_n, 
         all_of(species_cols_ordered))
...
ggplot() +
  geom_line(
    data = regen_lookup,
    aes(x = N.Regen.classes.present, y = value),
    color = "black", size = vf_size, alpha = vf_alpha
  ) +
  geom_point(
    data = regen_summary,
...
ggplot() +
  geom_line(
    data = native_canopy_lookup,
    aes(x = Native.Canopy.Percentage, y = value),
    color = "black", size = vf_size, alpha = vf_alpha
  ) +
  geom_point(
...
ggplot() +
  geom_line(
    data = vertical_structure_lookup,
    aes(x = N.Vertical.Structure.Categories, y = value),
    color = "black", size = vf_size, alpha = vf_alpha
  ) +
  geom_point(
...
ggplot() +
  ...
  geom_line(
    data = invasives_lookup,
    aes(
      x = reorder(domin, value),
      y = value,
      group = 1
    ),
    color = "black",
...
ggplot() +
  geom_line(
    data = microhabitats_lookup,
    aes(x = Proportion.of.microhabitats.present, y = value),
    color = "black", size = vf_size, alpha = vf_alpha
  ) +
  geom_point(
...
ggplot() +
  geom_line(
    data = Horizontal_complexity_lookup,
    aes(x = Horizontal.Complexity.Classes, y = value),
    color = "black", size = vf_size, alpha = vf_alpha
  ) +
  geom_point(
...
Note: value for the AVT indicator is calculated based on the mean AVT density across all plots, rather than the mean value across plots, as AVT condition operates at too large a scale to be captured by individual plots.
...
plotting.denses = data.frame(plot = veteran_trees_summary$plot,
                             dens.ests = veteran_trees_summary$dens_avt %>% 
                               jitter(0.1))
plotting.denses$dens.ests[plotting.denses$dens.ests > max(veteran_trees_lookup$N.Veteran.Trees.per.ha)] <- max(veteran_trees_lookup$N.Veteran.Trees.per.ha)


ggplot() +
  geom_line(
    data = veteran_trees_lookup,
    aes(x = N.Veteran.Trees.per.ha, y = value),
    color = "black", size = vf_size, alpha = vf_alpha
  ) +
  geom_point(
    aes(
...
ggplot() +
  geom_line(
    data = deadwood_lookup,
    aes(x = N.Deadwood.Categories/4, y = value),
    color = "black", size = vf_size, alpha = vf_alpha
  ) +
  geom_point(
...
ggplot() +
  geom_line(
    data = tree_health_lookup,
    aes(x = Dieback.or.sudden.mortality...., y = value),
    color = "black", size = vf_size, alpha = vf_alpha
  ) +
  geom_point(
...
domin_levels <- antropogenic_damage_lookup$domin

antropogenic_damage_lookup <- antropogenic_damage_lookup %>%
  mutate(
    domin = factor(domin, levels = domin_levels)
  )

anthropogenic_damage_summary <- anthropogenic_damage_summary %>%
  mutate(
    damage_cover = factor(
      damage_cover,
      levels = domin_levels
...
ggplot() +
  ...
  geom_line(
    data = antropogenic_damage_lookup,
    aes(
      x = domin,
      y = value,
      group = 1
  ...
  geom_point(
    data = anthropogenic_damage_summary,
    aes(
      x = damage_cover,
      y = value,
      fill = plot
  ...
  scale_fill_manual(
    values = wt_palette[-c(1,2,4)]
  ) +

  labs(
    x = "Anthropogenic damage cover (DOMIN)",
    y = "Value",
    title = paste(
      this.sheet,
...
Ground flora value is a function of the proportion of the appropriate ground flora species list present in each plot. The number appropriate ground flora species in the list varies by habitat.

For (`r survey_data[[1]]$survey_info$habiat_type`) there are `r length(survey_data[[1]]$indicators$ground_flora$appropriate_ground_flora)` appropriate species in the list:, they are: `r paste(sort(survey_data[[1]]$indicators$ground_flora$appropriate_ground_flora), collapse = ", ")`.

```{r ground_flora_plot, warning=FALSE, message=FALSE}
ggplot() +
  geom_line(
    data = ground_flora_lookup,
    aes(x = per_spp, y = value),
    color = "black", size = vf_size, alpha = vf_alpha
  ) +
  geom_point(
...
# Extract list of appropriate ground flora species
appropriate_gf <- survey_data[[1]]$indicators$ground_flora$appropriate_ground_flora

# Identify species columns in the dataset (everything except the fixed metadata columns)
gf_species_cols <- setdiff(names(ground_flora_summary),
                           c("plot", "value", "perc_of_appropriate_ground_flora",
                             "N_appropriate_ground_flora", "N_plant_spp"))

# Reorder so appropriate species come first
gf_species_cols_ordered <- c(
  gf_species_cols[tolower(gf_species_cols) %in% tolower(appropriate_gf) ],
  gf_species_cols[!tolower(gf_species_cols) %in% tolower(appropriate_gf) ]
)

# Rebuild the dataframe with new column order
ground_flora_summary_reordered <- ground_flora_summary %>%
  select(plot, value, perc_of_appropriate_ground_flora,
         N_appropriate_ground_flora, N_plant_spp,
         all_of(gf_species_cols_ordered))

# Build the datatable
datatable(
  ground_flora_summary_reordered %>% 
    mutate(
      value = round(value, 1),
      perc_of_appropriate_ground_flora = paste0(round(perc_of_appropriate_ground_flora, 1), "%"),
  ...
  # Highlight appropriate ground flora columns
  formatStyle(
    columns = gf_species_cols[tolower(gf_species_cols) %in% tolower(appropriate_gf)] %>% str_to_sentence(.),
